...
If the JS-selector is not ready to return a point (the expected event hasn't happend on the screen yet) it should throw [`ContinueError`](#class-continueerror), which will force the [`mouse`](Mouse%20actions.md) action to sleep a little bit and try to process the JS-selector one more time with another screenshot.
...
**Return value** - an object of the class [`TextTensor`](#class-texttensor), with an array containing all the matched text lines.

## find_text()

Same as `fine_text(text)`, but returns all the textlines found on the screen.

**Return value** - an object of the class [`TextTensor`](#class-texttensor), with an array containing all the text lines on the screen.
...
**Return value** - an object of the class [`ImgTensor`](#class-imgtensor), with all the images found matching the specified template.
...
Select the element with the specified index from the array of elements sorted from top to bottom (or from bottom to top or ... etc).
...
Return a Point with the coordinates of the geometric center of the textline (or the left bottom corner or ... etc).
...
The X (or Y) coordinate value of the Tensor's center.

> This property is accessible only when the current `Tensor` contains exactly one element. Otherwise an error is generated.

# Class TextTensor

Class `TextTensor` contains the information about strings on the screen. Could be obtained with a [`find_text`](#find_text) global function call. Derived from [`Tensor`](#class-tensor) class.
...
The same as `match_color(value, null)`

### match_background(value)

The same as `match_color(null, value)`

# Class ImgTensor

Class `ImgTensor` contains the information about images on the screen. Could be obtained with a [`find_img`](#find_imgpath_to_template) global function call. Derived from [`Tensor`](#class-tensor) class.
...
Returns a new Point with the `x` (or `y`) coordinate adjusted by N pixels.
...
The X (or Y) coordinate value of the point.
