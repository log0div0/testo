...
.. note::
	...
	С помощью аргумента командной строки ``test_spec <wildcard match>`` можно ограничить множество тестов для запуска. В случае указания этого аргумента будут запущены все тесты, подходящие под регулярное выражение, указанное в ``<wildcard match>``, а **также все их потомки**. Например, указав аргумент ``test_spec T4``, будут запущены тесты ``T4``, ``T7`` и ``T9`` (при условии, что все эти тесты могут быть запущены сразу и не требуют подготовительных действий). Если при этом запуск потомков нежелателен, то его можно исключить с помощью аргумента ``exclude <wildcard match>``
