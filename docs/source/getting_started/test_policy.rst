...
.. note::
	...
	При потере тестом кеша интерпретатор Testo в интерактивном режиме запрашивает у пользователя подтверждение запуска этого теста (а также всех его потомков). Это сделано для того, чтобы исключить повторный прогон тестов (что может занимать значительное время) в том случае, если потеря кеша была нежелательной (например, пользователь забыл указать значение переменной окружения, которая фигурирует в тесте). Это поведение можно изменить с помощью аргумента командной строки ``cache_miss_policy``.
