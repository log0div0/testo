...
**Описание**
	Дождаться появления на экране выражения, указанного в ``<select_expr>``. Длительность ожидания зависит от параметра ``timeout``. Если в течении указанного периода времени нужное выражение не обнаруживается, генерируется ошибка. Для работы действия виртуальная машина должна быть запущена.
...
Для более сложных проверок в команды ``wait`` и ``check`` можно передавать запросы, написанные на языке ``tql``, который структурно достаточно схож с языком ``sql``. Запросы необходимо заключать в обратные кавычки

.. code-block:: none

	wait `select * from textlines
		where text = "Hello world" and foreground = "blue" and background = "gray"`
...
.. warning ::
	...
	Язык ``tql`` в настоящее время находится на ранней стадии проектирования и будет, видимо, неоднократно переделываться. Сейчас допускается использовать запросы для поиска текста с заданным цветом букв и фона (как приведено в примере). Другие проверки работать не будут.
...
.. code-block:: none
	...
	wait !"Hello world" || `select * from textlines
		where text = "Menu entry"
			and foreground = "${foreground_colour}"
			and background = "gray"`

Дождаться либо отсутствия на экране надписи "Hello world", либо наличия на экране надписи "Menu entry" с серым фоном и цветом букв, определенным в переменной ``foreground_colour``
