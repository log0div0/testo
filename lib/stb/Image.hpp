...
struct RGB {
	...
	static uint8_t blend(uint8_t x1, uint8_t x2, uint8_t alpha_) {
		float alpha = float(alpha_) / 255.0f;
		float res = x1 * (1.0f - alpha) + x2 * alpha;
		if (res > 255.) {
			res = 255.;
		}
		return res;
	...
	void blend(const RGB& other, uint8_t alpha) {
		r = blend(r, other.r, alpha);
		g = blend(g, other.g, alpha);
		b = blend(b, other.b, alpha);
	}

	void blend(const RGB& other, const RGB& alpha) {
		r = blend(r, other.r, alpha.r);
		g = blend(g, other.g, alpha.g);
		b = blend(b, other.b, alpha.b);
	}

	void blend(const RGBA& other) {
		r = blend(r, other.r, other.a);
		g = blend(g, other.g, other.a);
...
struct Image {
	...
	Image sub_image(int off_x, int off_y, int sub_w, int sub_h) const {
		if (off_x < 0) {
	...
	template <typename T>
	void blend(int off_x, int off_y, const Pixel& pixel, const Image<T>& alpha) {
		if (off_x < 0) {
			throw std::runtime_error("off_x < 0");
		}
		if (off_y < 0) {
		...
		if ((off_x + alpha.w) > w) {
			throw std::runtime_error("(off_x + alpha.w) > w");
		}
		if ((off_y + alpha.h) > h) {
			throw std::runtime_error("(off_y + alpha.h) > h");
		}
		for (int y = 0; y < alpha.h; ++y) {
			for (int x = 0; x < alpha.w; ++x) {
				at(off_x + x, off_y + y).blend(pixel, alpha.at(x, y));
			}
		}
	}
