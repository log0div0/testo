...
import {TextLine, Text} from '../../common'
import {randomColors, randomInt} from '../../random'

function MyTextLine(props) {
	let colors = randomColors()
	let style = {
		fontSize: `${randomInt(8, 28)}pt`
	}
	return <TextLine color={colors.fg} backgroundColor={colors.bg} style={style}>{props.children}</TextLine>
}

function MyText(props) {
	let colors = randomColors()
	return <Text color={colors.fg} backgroundColor={colors.bg}>{props.children}</Text>
}
...
export function Example(props) {
	...
	return (
		<div id='background' style={{display: 'flex', flexDirection: 'column', paddingTop: '10px', paddingBottom: '10px'}}>
