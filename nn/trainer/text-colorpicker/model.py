...
def encode_colors(array):
	array = [colors.index(color) for color in array]
...
class Model(nn.Module):
	def __init__(self):
		...
		self.rnn = nn.Sequential(
			...
			BidirectionalLSTM(nh, nh, len(colors) * 2))
	...
	def forward(self, input):
		...
		fg = F.log_softmax(fg, dim=2)
		bg = F.log_softmax(bg, dim=2)

		return torch.cat([fg, bg], dim=2)
