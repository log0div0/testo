...
struct ActionException: public InterpreterException {
	...
	{
		msg = "Error while performing action " + std::string(*node);
		if (vmc) {
			msg += " on virtual machine ";
			msg += vmc->name();
		}
		msg += " at: " + std::string(node->begin());
	}
};
...
struct MacroException: public InterpreterException {
	explicit MacroException(std::shared_ptr<AST::MacroCall> macro_call):
		InterpreterException()
	{
		msg = std::string("In a macro call ") + macro_call->name().value() + " from here: " + std::string(macro_call->begin());
	}
};
