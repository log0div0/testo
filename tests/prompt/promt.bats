...
@test "prompt" {
	...
	echo $output | grep -A 1 "TEST TO RUN" | tail -n 1 | grep branch1_parent1
	echo $output | grep -A 2 "TEST TO RUN" | tail -n 1 | grep branch2_parent
	echo $output | grep -A 3 "TEST TO RUN" | tail -n 1 | grep branch1_parent2
	echo $output | grep -A 4 "TEST TO RUN" | tail -n 1 | grep branch1_child1
	echo $output | grep -A 5 "TEST TO RUN" | tail -n 1 | grep branch1_child2
	echo $output | grep -A 6 "TEST TO RUN" | tail -n 1 | grep branch2_child1
	echo $output | grep -A 7 "TEST TO RUN" | tail -n 1 | grep branch2_child2
