...
У виртуальной машины есть рядо атрибутов. Некоторые атрибуты являются обязательными, другие - опциональными. Обязательных атрибутов всего 4: `cpus`, `ram`, `disk_size` и `iso`. Рассмотрим их подробнее.
...
Мы видим, что несмотря на то, что мы не написали ни одного теста, такой файл все равно можно интепретировать. При этом, очевидно, никаких тестов не запускается.
