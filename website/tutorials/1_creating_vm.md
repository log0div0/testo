...
У виртуальной машины есть ряд атрибутов. Некоторые атрибуты являются обязательными, другие - опциональными. Обязательных атрибутов всего 4: `cpus`, `ram`, `disk` и `iso`. Рассмотрим их подробнее.
...
-   `disk main`: Настройки жесткого диска виртуальной машины. Для жёсткого диска обязательно необходимо указать его размер. Обратите внимание, что этот атрибут должен иметь имя - в нашем случае мы назвали жёсткий диск `main`. Мы могли бы создать несколько жёстких дисков.
